...
{
    Mat slice ((1+3)*300, ((int)(sqrt(5))+3)*300, CV_8UC3, Scalar(0, 0, 0));
    ...
    for(int i = 0; i < 100; i++)
    {
        ...
        //  cout<<maxPt.z<<" "<<minPt.z<<" "<<endl;
          ...
          int a = 2;
          int scale = 300;
          if(z < (maxPt.z - 0.10*body_height) && z > (minPt.z + 0.10*body_height))
          {
              slice.at<Vec3b>((int)((z+a)*scale), (int)((x+a)*scale))[0] = 255;
              slice.at<Vec3b>((int)((z+a)*scale), (int)((x+a)*scale))[1] = 255;
              slice.at<Vec3b>((int)((z+a)*scale), (int)((x+a)*scale))[2] = 255;
          }
          else
          {
              slice.at<Vec3b>((int)((z+a)*scale), (int)((x+a)*scale))[0] = 0;
              slice.at<Vec3b>((int)((z+a)*scale), (int)((x+a)*scale))[1] = 0;
              slice.at<Vec3b>((int)((z+a)*scale), (int)((x+a)*scale))[2] = 255;
          }
        }
...
{
    Mat slice ((1+3)*150, (1+3)*150, CV_8UC3, Scalar(0, 0, 0));
 ...
 //   allCurvates = getModelCurvatures(mesh);
    ...
    for (int i = 1; i < 99; i++)
    {
        for (int j = 0; j < body_cloud_filter->size(); j++)
        {
           ...
           if (abs(z - (maxPt.z - i*step)) < 0.1)
           {
               slice.at<Vec3b>((int)((x+a)*scale), (int)((y+a)*scale))[0] = 255;
               slice.at<Vec3b>((int)((x+a)*scale), (int)((y+a)*scale))[1] = 255;
               slice.at<Vec3b>((int)((x+a)*scale), (int)((y+a)*scale))[2] = 0;
           }
        }
     ...
     //   vector<vector<Point> > contours;
        ...
        threshold(gray_output, threshold_output, 15, 255, THRESH_BINARY );
   ...
   /*    imshow("Display", slice);
    ...
    float ratio_new = areas[max_area_index] / areas[15];                  //new ratio
    ...
    float ratio_height_length = body_height/max_length_rect  * 3;         // because the scale of width and length is 3
    float ratio_height_width = body_height/min_width_rect  * 3;
    ...
    output.push_back(ratio_length_width);
    output.push_back(ratio_height_length);
    output.push_back(ratio_height_width);
    output.push_back(ave_circularity);
    output.push_back(ratio_actual_ideal);
    output.push_back(max_length_index);
    ...
    for (int j = 0; j < body_cloud_filter->size(); j++)
    {
       ...
       {
            body_cloud_filter->points[j].r = 100;
            body_cloud_filter->points[j].g = 255;
            body_cloud_filter->points[j].b = 30;
       }
  ...
  /*  boost::shared_ptr<pcl::visualization::PCLVisualizer> viewer (new pcl::visualization::PCLVisualizer ("3D Viewer"));
   ...
   // viewer->addLine(top, bot, "line");
    ...
    return output;
   // vector<float> ratio_n;
   // ratio_n.push_back(ratio_new);
   // return ratio_n;
}
...
int main()
{
 //string directory = "potato_ply_mesh/small/";
 string directory = "temp/";
 char *shapes= &directory[0u];

 ofstream file;
 //file.open ("potato_shape_extra.txt", ios::app);
 file.open ("STR_K.txt", ios::app);
 ...
 {
    slice_name_side = "temp_side_images/" + cloud_names[i] + "-side.jpg";
    slice_name_top = "temp_top_images/" +  cloud_names[i] +  "-top.jpg";
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud (new pcl::PointCloud<pcl::PointXYZRGB>);
    string file_name = directory + cloud_names[i];
    ...
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr body_cloud_filter = outlier_remove(cloud, 300, 1);
    ...
    {
          body_cloud_filter->points[i].x = body_cloud_filter->points[i].x / body_height;
          body_cloud_filter->points[i].y = body_cloud_filter->points[i].y / body_height;
          body_cloud_filter->points[i].z = body_cloud_filter->points[i].z / body_height;
     }


  //  vector<float> cir_area = side_slice_analysis(body_cloud_filter);
   // cout<<cloud_names[i]<< ", Area CV: "<<cir_area[0]<<", Area ratio: "<<cir_area[1]<<" Angle CV: "<<cir_area[2]<<endl;

    vector<float> dim_ratio_index = top_slice_analysis(body_cloud_filter);
    cout<<cloud_names[i]<<" , L/W: "<<dim_ratio_index[0]<<" H/L: "<<dim_ratio_index[1]<<" H/D: "<<dim_ratio_index[2]<<" Average circularity: "<<dim_ratio_index[3]<<" Centroid length: "<<dim_ratio_index[4]<<" Max length index: "<<dim_ratio_index[5]<<endl<<endl;

  //  file<<cloud_names[i]<<","<<cir_area[0]<<","<<cir_area[1]<<","<<cir_area[2]<<","<<dim_ratio_index[0]<<","<<dim_ratio_index[1]<<","<<dim_ratio_index[2]<<","<<dim_ratio_index[3]<<","<<dim_ratio_index[4]<<","<<dim_ratio_index[5]<<endl;

    file<<cloud_names[i]<<",";
    if ((i+1)%3 != 0)
    {
        file<<dim_ratio_index[4]<<",";
    }
    else
    {
        file<<dim_ratio_index[4]<<endl;
    }
